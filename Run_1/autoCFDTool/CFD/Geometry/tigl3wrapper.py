...
# This file is automatically created from tigl.h on 2022-03-09.
...
class TiglAxis(object):
    TIGL_NO_AXIS = 0
    TIGL_X_AXIS = 1
    TIGL_Y_AXIS = 2
    TIGL_Z_AXIS = 3
    _names = {}
    _names[0] = 'TIGL_NO_AXIS'
    _names[1] = 'TIGL_X_AXIS'
    _names[2] = 'TIGL_Y_AXIS'
    _names[3] = 'TIGL_Z_AXIS'
...
class TiglSymmetryAxis(object):
    ...
    _names[4] = 'TIGL_INHERIT_SYMMETRY'
...
class TiglGeometricComponentType(object):
    ...
    TIGL_COMPONENT_CONTROL_SURFACE_DEVICE = 24
    TIGL_COMPONENT_OTHER = 25
    _names = {}
    _names[0] = 'TIGL_COMPONENT_PLANE'
    _names[1] = 'TIGL_COMPONENT_FUSELAGE'
    _names[2] = 'TIGL_COMPONENT_WING'
    _names[3] = 'TIGL_COMPONENT_SEGMENT'
    _names[4] = 'TIGL_COMPONENT_WINGSEGMENT'
    _names[5] = 'TIGL_COMPONENT_FUSELSEGMENT'
    _names[6] = 'TIGL_COMPONENT_WINGCOMPSEGMENT'
    _names[7] = 'TIGL_COMPONENT_WINGSHELL'
    _names[8] = 'TIGL_COMPONENT_WINGRIB'
    _names[9] = 'TIGL_COMPONENT_WINGSPAR'
    _names[10] = 'TIGL_COMPONENT_WINGCELL'
    _names[11] = 'TIGL_COMPONENT_GENERICSYSTEM'
    _names[12] = 'TIGL_COMPONENT_ROTOR'
    _names[13] = 'TIGL_COMPONENT_ROTORBLADE'
    _names[14] = 'TIGL_COMPONENT_ATTACHED_ROTORBLADE'
    _names[15] = 'TIGL_COMPONENT_PRESSURE_BULKHEAD'
    _names[16] = 'TIGL_COMPONENT_CROSS_BEAM_STRUT'
    _names[17] = 'TIGL_COMPONENT_CARGO_DOOR'
    _names[18] = 'TIGL_COMPONENT_LONG_FLOOR_BEAM'
    _names[19] = 'TIGL_COMPONENT_EXTERNAL_OBJECT'
    _names[20] = 'TIGL_COMPONENT_FARFIELD'
    _names[21] = 'TIGL_COMPONENT_ENGINE_PYLON'
    _names[22] = 'TIGL_COMPONENT_ENGINE_NACELLE'
    _names[23] = 'TIGL_COMPONENT_FUSELAGE_WALL'
    _names[24] = 'TIGL_COMPONENT_CONTROL_SURFACE_DEVICE'
    _names[25] = 'TIGL_COMPONENT_OTHER'


class TiglGeometricComponentIntentFlags(object):
    TIGL_INTENT_PHYSICAL = 1
    TIGL_INTENT_LOGICAL = 2
    TIGL_INTENT_INNER_STRUCTURE = 4
    TIGL_INTENT_OUTER_AERO_SURFACE = 8
    _names = {}
    _names[1] = 'TIGL_INTENT_PHYSICAL'
    _names[2] = 'TIGL_INTENT_LOGICAL'
    _names[4] = 'TIGL_INTENT_INNER_STRUCTURE'
    _names[8] = 'TIGL_INTENT_OUTER_AERO_SURFACE'
...
class TiglControlSurfaceType(object):
    TRAILING_EDGE_DEVICE = 0
    LEADING_EDGE_DEVICE = 1
    SPOILER = 2
    _names = {}
    _names[0] = 'TRAILING_EDGE_DEVICE'
    _names[1] = 'LEADING_EDGE_DEVICE'
    _names[2] = 'SPOILER'
...
class TiglShapeModifier(object):
    UNMODIFIED_SHAPE = 0
    SHARP_TRAILINGEDGE = 1
    BLUNT_TRAILINGEDGE = 2
    _names = {}
    _names[0] = 'UNMODIFIED_SHAPE'
    _names[1] = 'SHARP_TRAILINGEDGE'
    _names[2] = 'BLUNT_TRAILINGEDGE'
...
class TiglGetPointBehavior(object):
    asParameterOnSurface = 0
    onLinearLoft = 1
    numGetPointBehaviors = 2
    _names = {}
    _names[0] = 'asParameterOnSurface'
    _names[1] = 'onLinearLoft'
    _names[2] = 'numGetPointBehaviors'
...
class Tigl3(object):
    ...
    try:
        if sys.platform == 'win32':
            name = ctypes.util.find_library("tigl3")
    ...
    def wingSetGetPointBehavior(self, behavior):
        # input arg conversion
        _c_behavior = ctypes.c_int(behavior)

        # call to native function
        errorCode = self.lib.tiglWingSetGetPointBehavior(self._handle, _c_behavior)
        catch_error(errorCode, 'tiglWingSetGetPointBehavior', behavior)
    ...
    def wingInterpolateXsi(self, firstUID, firstEta, firstXsi, secondUID, secondEta, secondXsi, intersectionUID, intersectionEta):
        # input arg conversion
        _c_firstUID = ctypes.c_char_p(encode_for_c(firstUID))
        _c_firstEta = ctypes.c_double(firstEta)
        _c_firstXsi = ctypes.c_double(firstXsi)
        _c_secondUID = ctypes.c_char_p(encode_for_c(secondUID))
        _c_secondEta = ctypes.c_double(secondEta)
        _c_secondXsi = ctypes.c_double(secondXsi)
        _c_intersectionUID = ctypes.c_char_p(encode_for_c(intersectionUID))
        _c_intersectionEta = ctypes.c_double(intersectionEta)

        # output arg preparation
        _c_intersectionXsi = ctypes.c_double()
        _c_hasWarning = ctypes.c_int()

        # call to native function
        errorCode = self.lib.tiglWingInterpolateXsi(self._handle, _c_firstUID, _c_firstEta, _c_firstXsi, _c_secondUID, _c_secondEta, _c_secondXsi, _c_intersectionUID, _c_intersectionEta, ctypes.byref(_c_intersectionXsi), ctypes.byref(_c_hasWarning))
        catch_error(errorCode, 'tiglWingInterpolateXsi', firstUID, firstEta, firstXsi, secondUID, secondEta, secondXsi, intersectionUID, intersectionEta)

        _py_intersectionXsi = _c_intersectionXsi.value
        _py_hasWarning = _c_hasWarning.value

        return (_py_intersectionXsi, _py_hasWarning)
    ...
    def getControlSurfaceCount(self, componentSegmentUID):
        # input arg conversion
        _c_componentSegmentUID = ctypes.c_char_p(encode_for_c(componentSegmentUID))

        # output arg preparation
        _c_numControlSurfaces = ctypes.c_int()

        # call to native function
        errorCode = self.lib.tiglGetControlSurfaceCount(self._handle, _c_componentSegmentUID, ctypes.byref(_c_numControlSurfaces))
        catch_error(errorCode, 'tiglGetControlSurfaceCount', componentSegmentUID)

        _py_numControlSurfaces = _c_numControlSurfaces.value

        return _py_numControlSurfaces



    def getControlSurfaceUID(self, componentSegmentUID, controlSurfaceIndex):
        # input arg conversion
        _c_componentSegmentUID = ctypes.c_char_p(encode_for_c(componentSegmentUID))
        _c_controlSurfaceIndex = ctypes.c_int(controlSurfaceIndex)

        # output arg preparation
        _c_controlSurfaceUID = ctypes.c_char_p()

        # call to native function
        errorCode = self.lib.tiglGetControlSurfaceUID(self._handle, _c_componentSegmentUID, _c_controlSurfaceIndex, ctypes.byref(_c_controlSurfaceUID))
        catch_error(errorCode, 'tiglGetControlSurfaceUID', componentSegmentUID, controlSurfaceIndex)

        _py_controlSurfaceUID = decode_for_py(_c_controlSurfaceUID.value)

        return _py_controlSurfaceUID



    def getControlSurfaceType(self, controlSurfaceUID):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))

        # output arg preparation
        _c_controlSurfaceType = ctypes.c_int()

        # call to native function
        errorCode = self.lib.tiglGetControlSurfaceType(self._handle, _c_controlSurfaceUID, ctypes.byref(_c_controlSurfaceType))
        catch_error(errorCode, 'tiglGetControlSurfaceType', controlSurfaceUID)

        _py_controlSurfaceType = _c_controlSurfaceType.value

        return _py_controlSurfaceType



    def controlSurfaceGetMinimumControlParameter(self, controlSurfaceUID):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))

        # output arg preparation
        _c_minParameter = ctypes.c_double()

        # call to native function
        errorCode = self.lib.tiglControlSurfaceGetMinimumControlParameter(self._handle, _c_controlSurfaceUID, ctypes.byref(_c_minParameter))
        catch_error(errorCode, 'tiglControlSurfaceGetMinimumControlParameter', controlSurfaceUID)

        _py_minParameter = _c_minParameter.value

        return _py_minParameter



    def controlSurfaceGetMinimumDeflection(self, controlSurfaceUID):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))

        # output arg preparation
        _c_minDeflection = ctypes.c_double()

        # call to native function
        errorCode = self.lib.tiglControlSurfaceGetMinimumDeflection(self._handle, _c_controlSurfaceUID, ctypes.byref(_c_minDeflection))
        catch_error(errorCode, 'tiglControlSurfaceGetMinimumDeflection', controlSurfaceUID)

        _py_minDeflection = _c_minDeflection.value

        return _py_minDeflection



    def controlSurfaceGetMaximumControlParameter(self, controlSurfaceUID):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))

        # output arg preparation
        _c_maxParameter = ctypes.c_double()

        # call to native function
        errorCode = self.lib.tiglControlSurfaceGetMaximumControlParameter(self._handle, _c_controlSurfaceUID, ctypes.byref(_c_maxParameter))
        catch_error(errorCode, 'tiglControlSurfaceGetMaximumControlParameter', controlSurfaceUID)

        _py_maxParameter = _c_maxParameter.value

        return _py_maxParameter



    def controlSurfaceGetMaximumDeflection(self, controlSurfaceUID):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))

        # output arg preparation
        _c_maxDeflection = ctypes.c_double()

        # call to native function
        errorCode = self.lib.tiglControlSurfaceGetMaximumDeflection(self._handle, _c_controlSurfaceUID, ctypes.byref(_c_maxDeflection))
        catch_error(errorCode, 'tiglControlSurfaceGetMaximumDeflection', controlSurfaceUID)

        _py_maxDeflection = _c_maxDeflection.value

        return _py_maxDeflection



    def controlSurfaceGetControlParameter(self, controlSurfaceUID):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))

        # output arg preparation
        _c_controlParameter = ctypes.c_double()

        # call to native function
        errorCode = self.lib.tiglControlSurfaceGetControlParameter(self._handle, _c_controlSurfaceUID, ctypes.byref(_c_controlParameter))
        catch_error(errorCode, 'tiglControlSurfaceGetControlParameter', controlSurfaceUID)

        _py_controlParameter = _c_controlParameter.value

        return _py_controlParameter



    def controlSurfaceGetDeflection(self, controlSurfaceUID):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))

        # output arg preparation
        _c_deflection = ctypes.c_double()

        # call to native function
        errorCode = self.lib.tiglControlSurfaceGetDeflection(self._handle, _c_controlSurfaceUID, ctypes.byref(_c_deflection))
        catch_error(errorCode, 'tiglControlSurfaceGetDeflection', controlSurfaceUID)

        _py_deflection = _c_deflection.value

        return _py_deflection



    def controlSurfaceSetControlParameter(self, controlSurfaceUID, controlParameter):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))
        _c_controlParameter = ctypes.c_double(controlParameter)

        # call to native function
        errorCode = self.lib.tiglControlSurfaceSetControlParameter(self._handle, _c_controlSurfaceUID, _c_controlParameter)
        catch_error(errorCode, 'tiglControlSurfaceSetControlParameter', controlSurfaceUID, controlParameter)



    def controlSurfaceSetDeflection(self, controlSurfaceUID, deflection):
        # input arg conversion
        _c_controlSurfaceUID = ctypes.c_char_p(encode_for_c(controlSurfaceUID))
        _c_deflection = ctypes.c_double(deflection)

        # call to native function
        errorCode = self.lib.tiglControlSurfaceSetDeflection(self._handle, _c_controlSurfaceUID, _c_deflection)
        catch_error(errorCode, 'tiglControlSurfaceSetDeflection', controlSurfaceUID, deflection)
    ...
    def fuselageSetGetPointBehavior(self, behavior):
        # input arg conversion
        _c_behavior = ctypes.c_int(behavior)

        # call to native function
        errorCode = self.lib.tiglFuselageSetGetPointBehavior(self._handle, _c_behavior)
        catch_error(errorCode, 'tiglFuselageSetGetPointBehavior', behavior)
    ...
    def setExportOptions(self, exporter_name, option_name, option_value):
        # input arg conversion
        _c_exporter_name = ctypes.c_char_p(encode_for_c(exporter_name))
        _c_option_name = ctypes.c_char_p(encode_for_c(option_name))
        _c_option_value = ctypes.c_char_p(encode_for_c(option_value))

        # call to native function
        errorCode = self.lib.tiglSetExportOptions(_c_exporter_name, _c_option_name, _c_option_value)
        catch_error(errorCode, 'tiglSetExportOptions', exporter_name, option_name, option_value)



    def exportComponent(self, uid, fileName, deflection):
        # input arg conversion
        ...
        # call to native function
        errorCode = self.lib.tiglExportComponent(self._handle, _c_uid, _c_fileName, _c_deflection)
        catch_error(errorCode, 'tiglExportComponent', uid, fileName, deflection)



    def exportConfiguration(self, fileName, fuseAllShapes, deflection):
        # input arg conversion
        _c_fileName = ctypes.c_char_p(encode_for_c(fileName))
        _c_fuseAllShapes = ctypes.c_int(fuseAllShapes)
        _c_deflection = ctypes.c_double(deflection)

        # call to native function
        errorCode = self.lib.tiglExportConfiguration(self._handle, _c_fileName, _c_fuseAllShapes, _c_deflection)
        catch_error(errorCode, 'tiglExportConfiguration', fileName, fuseAllShapes, deflection)
    ...
    def componentGetType(self, componentUID):
        # input arg conversion
        _c_componentUID = ctypes.c_char_p(encode_for_c(componentUID))

        # output arg preparation
        _c_typePtr = ctypes.c_int()

        # call to native function
        errorCode = self.lib.tiglComponentGetType(self._handle, _c_componentUID, ctypes.byref(_c_typePtr))
        catch_error(errorCode, 'tiglComponentGetType', componentUID)

        _py_typePtr = _c_typePtr.value
    ...
    def setDebugDataDirectory(self, directory):
        # input arg conversion
        _c_directory = ctypes.c_char_p(encode_for_c(directory))

        # call to native function
        self.lib.tiglSetDebugDataDirectory.restype = ctypes.c_void
        _c_ret = self.lib.tiglSetDebugDataDirectory(_c_directory)
    ...
    def configurationGetBoundingBox(self):

        # output arg preparation
        _c_minX = ctypes.c_double()
        _c_minY = ctypes.c_double()
        _c_minZ = ctypes.c_double()
        _c_maxX = ctypes.c_double()
        _c_maxY = ctypes.c_double()
        _c_maxZ = ctypes.c_double()

        # call to native function
        errorCode = self.lib.tiglConfigurationGetBoundingBox(self._handle, ctypes.byref(_c_minX), ctypes.byref(_c_minY), ctypes.byref(_c_minZ), ctypes.byref(_c_maxX), ctypes.byref(_c_maxY), ctypes.byref(_c_maxZ))
        catch_error(errorCode, 'tiglConfigurationGetBoundingBox')

        _py_minX = _c_minX.value
        _py_minY = _c_minY.value
        _py_minZ = _c_minZ.value
        _py_maxX = _c_maxX.value
        _py_maxY = _c_maxY.value
        _py_maxZ = _c_maxZ.value

        return (_py_minX, _py_minY, _py_minZ, _py_maxX, _py_maxY, _py_maxZ)
